...
async fn get_lastfm_artist_constellation_branch(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        lastfm::artist_constellation_branch(app, root_artist_id, branch_name, branch_mbid)
    })
    .await
    .map_err(|error| format!("Last.fm artist constellation task failed: {error}"))?
    .map_err(|error| error.to_string())
}
...
async fn get_artist_biography(
    ...
    artist_id: String,
    force_refresh: Option<bool>,
) -> Result<artist_biography::ArtistBiography, String> {
    tauri::async_runtime::spawn_blocking(move || {
        artist_biography::artist_biography(app, artist_id, force_refresh.unwrap_or(false))
    })
    .await
...
async fn list_library_update_artists(
    app: AppHandle,
    request: LibraryUpdateRequest,
) -> Result<LibraryUpdateArtistResponse, String> {
    tauri::async_runtime::spawn_blocking(move || updates::list_artists_for_app(&app, request))
        .await
        .map_err(|error| format!("Artist update history task failed: {error}"))?
        .map_err(|error| error.to_string())
}
...
) -> Result<DiscoveryChartSnapshot, String> {
    tauri::async_runtime::spawn_blocking(move || {
        db::discovery_chart_snapshot_for_app(&app, request)
    })
    .await
    .map_err(|error| format!("Discovery chart task failed: {error}"))?
    .map_err(|error| error.to_string())
}
...
) -> Result<DiscoveryDeepCutSnapshot, String> {
    tauri::async_runtime::spawn_blocking(move || {
        db::discovery_deep_cut_snapshot_for_app(&app, request)
    })
    .await
    .map_err(|error| format!("Discovery deep-cuts task failed: {error}"))?
    .map_err(|error| error.to_string())
}
...
) -> Result<DiscoveryShelfExplorerResponse, String> {
    tauri::async_runtime::spawn_blocking(move || {
        db::discovery_shelf_explorer_for_app(&app, request)
    })
    .await
    .map_err(|error| format!("Discovery shelf explorer task failed: {error}"))?
    .map_err(|error| error.to_string())
}
