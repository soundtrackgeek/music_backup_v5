...
#[derive(Debug)]
struct BillboardSingleTrackCandidate {
    track_id: i64,
    album_key: String,
...
#[derive(Debug)]
struct WeeklyChartTrackCandidate {
    track_id: i64,
    album_artist_key: String,
    year: Option<i32>,
    normalized_rating: Option<i32>,
...
fn ensure_ti_i_skuddet_schema(conn: &Connection) -> Result<()> {
    for (name, definition) in [
        ("ti_i_skuddet_rank", "INTEGER"),
        ("ti_i_skuddet_year", "INTEGER"),
        ("ti_i_skuddet_debut_date", "TEXT"),
...
fn import_ti_i_skuddet_singles(
    conn: &mut Connection,
    source_path: &Path,
) -> Result<TiISkuddetImportSummary> {
    let started = Instant::now();
...
fn chart_csv_files(source_path: &Path, source_label: &str) -> Result<Vec<PathBuf>> {
...
fn optional_chart_csv_header_index(headers: &csv::StringRecord, name: &str) -> Option<usize> {
    headers
        .iter()
...
fn billboard_single_source_album_key(album: &str, label_number: &str) -> String {
...
fn billboard_single_catalog_label_matches(album_suffix: &str, label_number: &str) -> bool {
...
fn billboard_single_album_match_score(library_album_key: &str, source_album_key: &str) -> i32 {
...
fn billboard_single_album_artist_matches(album_artist_key: &str, source_artist_key: &str) -> bool {
...
fn billboard_single_is_compilation_artist(album_artist_key: &str) -> bool {
    matches!(
        album_artist_key,
...
fn billboard_single_release_year_score(year: Option<i32>, debut_year: Option<i32>) -> i32 {
...
fn billboard_single_candidate_priority(
...
#[cfg(not(test))]
fn refresh_billboard_single_album_metadata_if_needed(conn: &mut Connection) -> Result<()> {
...
fn timeline_source_is_billboard(value: &str) -> bool {
...
fn year_progress_stats_filtered(
    conn: &Connection,
    request: &YearProgressRequest,
) -> Result<Vec<YearProgressStats>> {
