...
const mockStatistics: StatisticsResponse = {
  ...
  genreProgress: [
    ...
    const albumCount = 1840 - index * 47;
    const ratedAlbumCount = Math.round(albumCount * (0.18 + (index % 5) * 0.04));
    const partialAlbumCount = Math.round(albumCount * (0.1 + (index % 3) * 0.02));
    return {
      genre,
      albumCount,
      ratedAlbumCount,
      partialAlbumCount,
      unratedAlbumCount: albumCount - ratedAlbumCount - partialAlbumCount,
      trackCount: albumCount * (10 + (index % 4)),
      totalSeconds: albumCount * (2100 + (index % 6) * 90),
      lovedTracks: Math.round(albumCount * (0.12 + (index % 4) * 0.03)),
      averageAlbumScore: 82 + (index % 8) * 6.2,
    };
  }),
  lovedDensity: [
    {
