...
- Internet access is required to load Music Map tiles, enrich new MusicBrainz area/country coordinates, search or validate MusicBrainz Wish List additions and artist discographies, and validate/search a configured Deemix connection
...
The desktop dev shell loads Vite from `http://127.0.0.1:1420/`, matching the loopback host used by `npm run dev`. Vite ignores the local `musicbee-library.tsv` export, `AlbumCovers/` archive, US `CSV_ALBUMS/` and `CSV_SINGLES/` chart folders, Norwegian `CSV_ALBUMS_NO/` and `CSV_SINGLES_NO/` chart folders, and `MusicBrainz/` cache folder during development so large library data cannot stall the dev server watcher. If the Tauri window opens but stays blank, make sure port `1420` is free and restart `npm run tauri:dev`.
...
Chart-source wording stays source-specific. For example, `Give me all the nr. 3 albums in the VG Lista charts from 1980 to 1989` applies VG Lista rank 3 rather than Billboard rank 3, while `Find albums that were on VG Lista but not Billboard` combines a VG Lista presence filter with missing Billboard data. Track requests such as `Find Ti i Skuddet top 10 singles that debuted in summer 1989` use Ti i Skuddet rank and ISO debut-week fields without substituting VG Lista or Billboard. Luna supports source-specific presence/missing checks, ranges, sorting, chart ranking, and Playlist Builder recipes, and rejects Ti i Skuddet fields in album view because that list is singles-only.
...
Album wishes expose **Search with Deemix**. Each search revalidates the stored ARL, queries Deezer directly over HTTPS, ranks album candidates against the wish's artist, title, and year, and labels them Exact, Likely, or Possible. Candidate links are constructed from numeric Deezer album IDs. **Download album** authorizes the selected release through the connected Deezer account, keeps Deezer-issued session cookies in an ephemeral in-memory cookie jar, refreshes a rejected CSRF/API token once, requests each track in the configured preferred format with the saved fallback policy, decrypts each media stream locally, and reports live metadata/artwork/download/tagging progress.

Before a download starts, the app checks its local receipt and expected destination folder. An existing album produces an inline warning with the exact path; **Download another copy** remains available and creates a numbered sibling folder without overwriting files. Artist wishes with a MusicBrainz ID expose **Search official albums with Deezer**: the backend keeps only pure primary Album release groups with no secondary type and at least one Official MusicBrainz release, persists that verified snapshot for fast Wish List summaries, searches Deezer for each bounded result, and shows them in **Albums found**. A first uncached artist summary is loaded sequentially in the background. Albums can be queued individually while another is downloading, or **Download all albums** can queue every missing result with a Deezer match for sequential processing while skipping releases already present in the imported library or download receipts.
...
## Phase 11 US and Norwegian Chart Features
...
- VG Lista weekly album and single imports from CSV files in `CSV_ALBUMS_NO/` and `CSV_SINGLES_NO/` with `Year,Week,Rank,Last Week,Movement,Artist,Title,Weeks on Chart,Source URL` headers. The combined album and single import controls select US and Norway by default.
...
- Matched album and track rows also carry VG Lista peak rank and first chart ISO week; every raw weekly Norwegian row remains in a dedicated country-and-chart-type table so future weekly analysis does not depend on the summary fields.
- Search and Charts support Billboard, VG Lista, and track-only Ti i Skuddet rank/debut ranges, sorting, columns, and ranking metrics. A nested **Chart filters** disclosure keeps every source inside the existing advanced drawer; optional columns label each chart explicitly.
- The standalone **Timeline** workspace provides **Albums through the years** and **Tracks through the years** with Billboard and VG Lista sources plus track-only Ti i Skuddet, built-in seasons, Christmas, New Year, full-year, custom month periods, fullscreen presentation, titled artwork cards, metric/custom ordering, canonical single-copy track selection, and exact playlist handoff. Luna Search, Chart, and Playlist Builder recipes understand all source-specific rank/debut fields, including seasons such as `Ti i Skuddet singles that debuted in summer 1989`.
