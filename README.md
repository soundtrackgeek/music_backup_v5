...
Usenet work happens in an app-owned hidden staging folder beneath the selected download directory. A completed direct-file release is moved into `Artist - Album (Year)`; a RAR release is first verified and extracted with an installed UnRAR executable, including the detected `C:\Tools\UnRAR\UnRAR.exe` and standard WinRAR locations. Existing album folders are never overwritten, retries receive numbered sibling folders, and a failed or incomplete post never becomes a completed destination. PAR2 repair is not performed in this first native downloader version, so a release with unavailable or corrupt articles fails clearly and can be retried with another NZB.
...
Before a download starts, the app checks its local receipt and expected destination folder. An existing album produces an inline warning with the exact path; **Download another copy** remains available and creates a numbered sibling folder without overwriting files. Artist wishes with a MusicBrainz ID expose one official-album search: the backend keeps only pure primary Album release groups with no secondary type and at least one Official MusicBrainz release, persists that verified snapshot for fast Wish List summaries, and matches each bounded result with Deemix. Once **Albums found** appears, the app automatically searches Soulseek for every album still missing from the library and download receipts, using up to six concurrent searches while showing queued, searching, completed, empty, and failed states inline. Each missing album then offers **Download with Deemix** when a Deezer match exists and one or more ranked Soulseek release folders when peers answer. **Download all with Deemix** keeps the existing sequential bulk workflow; Soulseek downloads remain an explicit per-source choice.
