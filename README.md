...
Norwegian VG Lista albums and singles import from weekly CSV rows in `CSV_ALBUMS_NO/` and `CSV_SINGLES_NO/`. The album and single import actions each let US and Norway be selected independently, select both by default, and continue with the other country if one source fails. Every Norwegian weekly row is retained in its own chart table; matching library rows receive the best rank and earliest ISO chart week for fast Search, Charts, and Timeline use.
...
- Search and Charts support Billboard and VG Lista rank/debut ranges, sorting, columns, and ranking metrics. Charts can rank either albums or tracks; optional columns label each chart source explicitly.
- The standalone **Timeline** workspace provides **Albums through the years** and **Tracks through the years** with a US Billboard / NO VG Lista selector, built-in seasons, Christmas, New Year, full-year, custom month periods, fullscreen presentation, titled artwork cards, metric/custom ordering, canonical single-copy track selection, and exact playlist handoff. Luna Search, Chart, and Playlist Builder recipes understand Billboard date fields, including seasons such as `summer 1989`.
...
- SQLite schema version 10 adds persisted Billboard singles chart entries alongside nullable Billboard singles rank/year track fields; schema version 34 adds normalized `Date Entered` values, calendar/ISO-week fields, quality markers, and indexed track debut fields; schema version 35 adds raw and normalized source-album fields for canonical track matching; schema version 36 adds country-specific VG Lista weekly chart tables, match summaries, indexes, and saved source paths.
