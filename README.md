...
Charts follows the same progressive pattern. Search, Table/List/Grid/Timeline, ranking, direction, included and excluded genres, library year, and Billboard chart-debut week controls stay visible while the current ranking begins in the first screen. Timeline renders the filtered albums as selectable year-density bars with week-sorted album cards and clearly reports albums without debut data. Result limit, built-in presets, lifecycle, MusicBrainz, scoring, completeness, column, cover-size, and export controls live in a collapsed **Advanced chart controls** drawer with an active hidden-control count. Chart-building and current-ranking Luna tasks share one collapsed **Luna commands** area with **Build a chart** and **Ask this chart** modes.
...
- Search and Charts support Billboard chart-debut week from/to filters, debut-week sorting and columns, and a selectable **Albums through the years** timeline. Luna Search, Chart, and Playlist Builder recipes understand chart-debut periods and calendar seasons such as `summer 1989`.
