...
The compact conversation retains at most five completed exchanges in the current app session and sends no more than the latest eight validated turns. Changing the workspace or selected entity clears the conversation so facts from one selection cannot bleed into another. Music Research is not automatically saved as a snapshot; closing and reopening on the same context keeps the in-memory exchange until the selection changes or the user clears it.
