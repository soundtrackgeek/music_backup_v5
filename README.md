...
The fixed sparkle button beside the global top-right controls opens **Luna Music Research** from every workspace. It stays visible whether the right details sidebar is open or hidden. On Albums, Artists, and Genres, the panel attaches the currently selected album, artist, or genre as a context clue and shows that attachment before anything is sent. The selection guides the question without restricting it: a question can be directly about the selection, use it as a starting point, compare it with something else, or move into wider music research. Search and Charts deliberately open in General music research mode and do not inherit those workspaces' separate Ask Luna filters or current-view analysis state.
