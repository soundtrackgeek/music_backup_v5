...
Library Completion is the command centre for finding charted albums that are missing from the imported collection. Its local scan compares normalized artist and album keys across Billboard 200, Official UK Albums, and VG Lista data, merges the evidence for albums found in more than one source, and ranks candidates without making any provider requests. The **Workbench** combines the queue with an evidence dossier, persistent **Wanted**, **Needs review**, and **Not for me** decisions, and direct handoffs to the existing Wish List and Deemix search flow.

The **Coverage Atlas** summarizes owned albums and open gaps by chart source and decade. Selecting a cell and choosing **Review candidates** starts a focused Workbench campaign for that cohort. MusicBrainz verification is deliberately manual and on demand. Discogs is visible as the next provider phase but is not contacted in version 0.96.0, so Discogs credentials are not yet stored or required. SQLite schema version 40 stores completion decisions and their optional Wish List and MusicBrainz links.
