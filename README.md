...
Album wishes expose **Search with Deemix**. Each search revalidates the stored ARL, queries Deezer directly over HTTPS, ranks album candidates against the wish's artist, title, and year, and labels them Exact, Likely, or Possible. Candidate links are constructed from numeric Deezer album IDs. **Download album** authorizes the selected release through the connected Deezer account, requests the exact configured bitrate without silently substituting a lower one, decrypts each media stream locally, and reports live metadata/artwork/download/tagging progress.
