...
Timeline opens **Albums through the years** as a dedicated dark experience. It maps dated Billboard album debuts across the collection, lets you move among chart years and Summer/Autumn/Winter/Spring/full-year views, select individual debut weeks and album covers, open the chosen album, hand the exact seasonal album cohort to Playlist Builder, jump to Search, and enter true fullscreen mode. The desktop composition follows the cinematic decade-ribbon design; narrow windows switch to a compact focus view with scrollable album artwork. Billboard first appearance is used as the historical marker and is not presented as a verified retail release date.
...
- Search and Charts support Billboard chart-debut week from/to filters, debut-week sorting, and columns. The standalone **Timeline** workspace provides **Albums through the years**, while Luna Search, Chart, and Playlist Builder recipes understand chart-debut periods and calendar seasons such as `summer 1989`.
