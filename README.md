...
The GUI and Aurora companion bridge use `~/Library/Application Support/com.local.musiclibrary/music-library.sqlite3`. With the apps closed, populate this with a consistent local catalog snapshot. Preserve the original snapshot as a backup. Windows music and import paths in the catalog are unchanged; this change does not enable remote catalog editing or automatic catalog synchronization. Aurora Network Mode handles mounted music paths for playback. Use the home PC for library import/edit operations during this initial Mac rollout.

A local-first desktop app for synchronizing already-tagged MP3 album folders, importing legacy MusicBee TSV snapshots, and searching, browsing, and analyzing the resulting music catalog.

Aurora album intake uses Music Library's reviewed add/replace/move-back transaction. Newly added and replacement releases run the existing cover-archive workflow, validate every published MP3, and save format, size, bitrate, duration, and scan outcomes to the existing Music Doctor quality tables after catalog commit. The bridge publishes live scan, fingerprint, transfer, verification, catalog, artwork, and cleanup progress for Aurora, including exact transferred files and bytes. Small batches reuse released SQLite staging pages instead of synchronously vacuuming the complete catalog, and redundant immediate destination re-hashes are replaced by exact structure checks without removing the independent pre-commit and post-commit SHA-256 verification. This lets Aurora own quality scanning for new music while the completed Music Doctor dataset remains the historical baseline. Intake quality validation treats ordinary and Windows verbatim forms of the same destination root as equivalent. Replacements preserve the catalog album identity and retain the verified old folder as a hidden recovery copy, while move-back copies an album to Aurora Inbox before removing it from the catalog and library root. Move-back treats ordinary and Windows verbatim forms of the same validated source path as one folder. Verified single-album removal and move-back now delete only the selected catalog and search rows, retaining other track identities and ratings. Chart reconciliation is skipped when no removed records are linked; affected chart links still use the complete reconciler. The full recovery backup, source/destination verification, atomic commit, and post-commit cleanup remain in place. New **Remove Album** previews (Music Library 0.145.13 with Aurora 0.24.33) store only the selected album scope; apply rechecks those complete rows under the write lock and does not stage, compare, or rewrite retained tracks. Other albums may change without invalidating that scope. Removal-specific sessions cannot be applied as ordinary TSV imports. Existing older previews and Move to Inbox retain their original snapshot validation. The full recovery backup still scales with catalog size and uses a verified WAL checkpoint and a held SQLite write lock while copying a consistent image. Backup, search-entry removal, catalog rows, totals, chart links, and commit report separate phases. Smart playlists refresh through the existing playlist-load and Plex-sync paths instead of delaying every removal. Unrelated chart state that was already stale stays visibly stale for the existing chart-rebuild flow; linked removed records are still reconciled. Automatic folder sync cannot start whole-catalog fallback staging. MusicBee extended-text fields accept trailing NUL terminators during preview scanning, including LOVE RATING, without rewriting the source tags; malformed and duplicate love values remain blocked. The release gate serializes Rust tests that share the process-wide import workflow guard so validation remains deterministic.
...
The same Overview also loads `artist.getSimilar` and presents up to 12 listener-related artists in **In your library** and **Explore** groups. Local matches prefer MusicBrainz MBIDs and fall back to normalized artist names; owned cards reuse local portraits or representative album covers and open the local Artist page, while missing artists and the complete result link open Last.fm. Missing cards can also be added directly to the Wish List: an exact MusicBrainz match is retained when available, normalized duplicates are suppressed, the source artist remains visible as provenance, and the wish automatically completes when a later import adds that artist locally. Match percentages are Last.fm relationship scores, not local play counts. An optional accessible **Artist constellation** preserves that shelf and visualizes its first eight results; selecting a first-hop artist can explicitly expand up to six second-hop connections, with at most three branches per root artist and no third hop. Expansion is serialized, reuses the schema-52 relationship cache, and keeps stale cached branches usable offline. Solid nodes are owned locally, dashed nodes are missing, arrow keys traverse the graph, and the equivalent list view provides direct local or Last.fm navigation. Discovery reuses these cached relationships for its **Because You Played / Loved** shelf and falls back to canonical genre links when an anchor has not been enriched yet.

Album pages derive up to 12 **Related Albums** from documented Last.fm API data: `album.getTopTags`, `tag.getTopAlbums`, and the existing `artist.getSimilar` relationship signal. Results are ranked by shared album tags with a similar-artist boost, then split into **In your library** and **Explore**. Each card shows the tags and artist relationship that support it; owned albums open locally and missing albums retain their Last.fm link plus a direct Wish List action. The action uses an exact normalized MusicBrainz release-group match when available, prevents duplicate artist/title or MBID wishes, records the recommending source album, and hands off to local Album navigation after import. This is an API-derived recommendation, not a copy of Last.fm's private website ranking.
...
Search and Charts also include **Ask about this view**. Questions such as `Which artists appear most often?`, `How many are unrated?`, or `What stands out about these albums?` operate on a snapshot of the active filters. Search questions inspect all filtered matches unless the question explicitly asks for the top/first N in the active ordering. Chart questions stay inside the displayed ranked result limit, and an explicit smaller top/first N narrows every overview, group, and named-list inspection to those same leading rows. Luna must call one strict `inspect_current_view` tool; the desktop app then executes only validated local SQLite operations. A question can request one to three compact inspections: an exact overview, up to 20 grouped values, and/or up to 50 named albums or tracks. File paths, filenames, database files, covers, saved objects, the complete result set, and arbitrary SQL are never sent. Album, track, and artist names are sent only when the user explicitly asks a question whose answer needs a bounded named list.
...
**Statistics → Rating progress** opens the Year Ledger. Select one or several canonical genres (combined as OR), optionally exclude genres, then enter an exact **album Year** range or choose a decade. Year is separate from Release year. Commit year inputs with Enter or by leaving the field. Every year in the inclusive range appears in chronological order, including years with no matching albums; scroll the ledger for long ranges. Albums without a Year are excluded. Green represents fully rated albums (all tracks rated), amber represents partial albums, and red represents unrated albums. **Left to finish** is partial plus unrated. Every bar uses a fixed 0–100% scale: green, amber, and red show the fully rated, partial, and unrated shares of that year’s albums. The green share matches **Complete %**; years with no albums have an empty bar. Album counts remain in the adjacent columns, and segment tooltips show both counts and percentages. Click a year for its summary, or a colored segment/count to open those albums in Search. **Browse remaining albums** includes every album below 100% completeness. The selected genres, year range, and year remain when returning from Search. Existing dashboards remain under **Overview**.
...
Use **Wish List** to keep wanted artists and albums separate from the music already in your collection. **Add artist or album** searches MusicBrainz directly from this workspace and lets you choose the correct catalog match before anything is saved. Artist additions request only Album release groups, then verify the selected artist's official pure Album releases—primary type Album with no Compilation, Live, Remix, DJ-mix, Soundtrack, or other secondary type—against imported albums and completed downloads; EP and other release-group categories are not required, and an artist with no missing official albums—or no official albums at all—is explained and not added. Album additions re-confirm that the selected MusicBrainz release group still exists, is a pure primary Album with no secondary type, and has at least one Official release before saving it. You can also add an album from a selected artist's **MusicBrainz Discography**, add an artist/album from a **Find what your library is missing** result, or save missing Similar Artist and Related Album recommendations in place. Duplicate MusicBrainz and normalized name identities are ignored, recommendation provenance is shown on each item, each item can be removed manually, and MusicBrainz artist/release-group links remain available from the list.

The list is local SQLite data included in normal database backups. Whenever Wish List opens—and immediately after a successful MusicBee import—the app compares album wishes against the current library and removes albums you have acquired. Artist wishes created as general MusicBrainz discography trackers remain after the artist enters the library; Similar Artist recommendation wishes complete once that artist appears locally. Each persistent artist card shows its current missing official-album count, and an eye control reveals the missing album titles and years on hover or keyboard focus. This viewport-aware panel opens above or below the control as space permits, escapes card clipping, and scrolls internally for long discographies. The count includes normalized albums already imported into the library plus completed Deemix receipts. Completed Deemix downloads also retain local receipts so album wishes and artist-discography rows can show a persistent **Downloaded** badge. SQLite schema version 24 adds Wish List storage, while schema version 31 adds the download receipts.
...
### Tagged album folder sync

On **Imports**, choose one complete album folder after you have tagged, renamed, and placed it in its final library location. Music Library reads MP3 ID3 fields directly: title, artist, `DISPLAY ARTIST`, album artist, album, track/disc number, genre, publisher, year, native or legacy `TDRL` release year, `LOVE RATING`, MusicBee-owned POPM half-star rating, verified legacy `Default`-owned POPM whole-star rating, and duration. An explicit MusicBee rating takes precedence over the legacy fallback, and duplicate or unsupported rating frames fail closed. It does not search MusicBrainz or Discogs and it does not rename or move files.

The selected folder may contain disc subfolders but must resolve to one album and album artist. Linked path ancestors, mixed albums, duplicate MusicBee rating frames, and files other than MP3s or recognized artwork, playlist, checksum, and liner-note sidecars are rejected instead of being imported partially. Existing catalog identity is retained when the folder is already known; new albums receive an app-owned stable identity. Album Rating is calculated as the floored mean of the rated tracks, matching the observed MusicBee behavior for partially rated albums.

This first MusicBee-free bridge is deliberately a whole-catalog operation, not a direct incremental database writer. Music Library generates a compatibility snapshot from the active normalized tracks outside the selected folder plus the newly scanned MP3 rows, so app-owned catalog repairs are not reverted, then runs the existing staged delta preview. The active database remains live during preparation; suspicious removals are visible before apply. Before apply, Music Library rechecks the exact folder paths, ID3 bytes, and active catalog content; after acquiring the atomic write transaction it rechecks the folder and verifies that no catalog commit raced with that preflight. A stale preview is refused, and a rollback backup already exists before replacement starts. Embedded cover art remains a separate cover-import step. The legacy MusicBee TSV route remains available by entering a `.tsv` path.
...
Apply revalidates the plan, catalog, source bytes, and destinations, then copies every album to an owned hidden staging folder on the destination volume. For replacements it atomically moves the verified original to a hidden recovery path before publishing the new folder; any pre-commit failure removes the owned new copy and restores that original. The commit guard validates the unchanged source, published copy, preserved recovery folder, and catalog state without comparing the newly published destination to the original release fingerprint. For move-back, the verified Inbox copy is published before the normal importer commits the album removal. Sources are isolated and deleted only after commit. Completed replacement receipts return the recovery path; cleanup uncertainty is reported without deleting unverifiable data.

`syncExistingFolders` accepts 1–32 canonical folders already present in the active catalog and synchronizes tag changes without moving files. Aurora may also supply an exact changed MP3 path: Music Library validates that it is a regular, unlinked, cataloged file inside the requested album and scans only that track for the common one-file path. Missing, multiple, ambiguous, stale, or unsupported targets retain the complete-folder safety path. Every scanner path uses the same MusicBee-first, verified legacy-`Default` POPM fallback, and the bridge advertises `defaultPopmRatingFallback` so Aurora can refuse an older unsafe helper while keeping its durable sync queue pending. Service callers can require `serviceExistingFolderSync`: existing-folder sync treats each configured absolute root as a containment boundary without separately enumerating that parent directory, avoiding false root-unavailable failures under a Windows service while still canonicalizing and link-checking the exact album folder and MP3. Ordinary and Windows verbatim drive prefixes are equivalent for that boundary match, so `D:\...` and `\\?\D:\...` resolve to the same configured volume. The bridge rejects paths outside configured library roots, linked/reparse paths, unsupported files, additions or removals, identity-set changes, and mutations outside the selected scope. When the exact catalog identity is unchanged and the edit is limited to rating, Love/Ban, or Release Year, Music Library updates only that album's existing raw track, normalized track, and aggregate rows in one guarded transaction, preserves track IDs, stored duration, and chart metadata, and records the completed import, Updates history, rating event, and whole-library rating snapshot without a whole-catalog stage or backup. It returns as soon as that transaction commits: wish-list membership cannot change during this scoped edit, and smart playlists already reevaluate current catalog rows when opened, refreshed, or synchronized to Plex. A scanner duration within one second of the stored value is treated as harmless rounding and does not rewrite catalog time. Blank catalog disc numbers and zero-valued MP3 disc tags are also equivalent for this guard, so legacy `disc=0/0` tags do not force a full-catalog fallback or alter the stored disc metadata. Any other metadata change or uncertain precondition returns a reviewed-reconciliation error without staging the catalog or creating a backup. Aurora 0.24.35 blocks that queued edit without retrying each overlay track, retaining the file edits and local overlays until reconciliation and a later retry. Broader changes require the reviewed Music Library import flow. Schema 57 first repairs canonical legacy half-star rows and their album aggregates so unrelated historical data cannot invalidate a correctly scoped edit. Discarded full-catalog staging is checkpointed promptly, and exact deprecated `music-library-aurora-sync-<24 hex characters>-before-import.sqlite3` backups are pruned to the newest safety copy without touching ordinary backups. Exact Aurora-owned `.<MP3 filename>.aurora-<positive operation id>.original.backup` recovery files are ignored during scanning; other `.backup` files still fail closed.

The bridge derives `%APPDATA%\com.local.musiclibrary\music-library.sqlite3` itself and does not require a Tauri window or `AppHandle`. For isolated development and tests only, `MUSIC_LIBRARY_BRIDGE_APP_DATA_DIR` can point bridge mode at an absolute temporary app-data directory; when it is set, `MUSIC_LIBRARY_BRIDGE_GENERAL_ROOT`, `MUSIC_LIBRARY_BRIDGE_SCORES_ROOT`, and `MUSIC_LIBRARY_BRIDGE_SYNTHWAVE_ROOT` are also required so an isolated test cannot fall through to a live category destination. Requests and responses use unique files; the response envelope is always `{ "protocolVersion": 1, "ok": true, "result": ... }` or `{ "protocolVersion": 1, "ok": false, "error": { "code": ..., "message": ... } }`.

The import screen also manages `AlbumCovers`, US `CSV_ALBUMS` and `CSV_SINGLES`, UK `CSV_ALBUMS_UK` and `CSV_SINGLES_UK`, and Norwegian `CSV_ALBUMS_NO`, `CSV_SINGLES_NO`, `CSV_TIISKUDDET_NO`, and `CSV_NORSKTOPPEN_NO`. When a US chart source is a full path and a country-specific source still has its bare default name, the corresponding UK and Norwegian sources expand to sibling paths automatically—for example, `C:\Music\CSV_SINGLES` produces `C:\Music\CSV_SINGLES_UK`, `C:\Music\CSV_SINGLES_NO`, `C:\Music\CSV_TIISKUDDET_NO`, and `C:\Music\CSV_NORSKTOPPEN_NO`. Editing any Imports source path saves it automatically after a short pause; **Save paths** remains available to save immediately or retry a failed write. The saved-state indicator reflects the last path values confirmed by the backend, and exact Windows paths—including a trailing backslash such as `C:\Music\AlbumCovers\`—are restored across app restarts. Existing settings that still contain the old exact `CSV` default migrate automatically to `CSV_ALBUMS`. SQLite schema version 20 stores the original Imports workspace paths, app-owned MusicBrainz artist origin-country and artist-info tables, the Origin Country flag/name display preference, and the portable unconfigured overlay-sync default; schema version 33 adds Billboard album first-appearance month/week data and indexed canonical ISO-week keys; schema version 34 adds Billboard single chart-entry dates, calendar parts, ISO weeks, source-date quality, and lookup indexes; schema version 35 adds Billboard single source-album provenance and normalized album matching keys; schema version 36 adds the two VG Lista paths, dedicated Norwegian weekly album and single tables, and indexed summary fields on matched library rows; schema version 37 adds the Ti i Skuddet path, dedicated weekly singles table, and indexed track summary fields; schema version 38 adds the Norsktoppen path, dedicated weekly singles table, and indexed track summary fields; schema version 39 adds the Official UK paths, dedicated weekly album and singles tables, matched-row summaries, and indexes; schema version 40 adds persistent Library Completion decisions and provider links; schema version 41 adds persistent Library Completion verification outcomes, resumable batches, and queue items; schema version 42 adds provider-specific MusicBrainz and Discogs evidence; schema version 43 adds independent Library Completion cover-enrichment state and cache metadata; schema version 44 adds persisted missing-chart-artist verification, decisions, and resumable batches. Relative paths are resolved from the app process directory and its parent, so repo-root source folders work during local development. MusicBee TSV quote characters are treated as literal tag text during preparation, matching plain TSV exports where titles can contain unpaired quotes. Date-like MusicBee `Year` and `Release Year` values such as `2019-06-28` are normalized to `2019`. Preparation writes resumable checkpoint rows into the app database but does not replace the active `tracks`, `albums`, or `raw_tracks` snapshot, so the SQLite file can temporarily grow by roughly the staged snapshot size. Cancel interrupts staging or the final delta analysis and keeps the last durable checkpoint for resume. Apply creates the rollback backup immediately before its atomic replacement transaction; Apply removes its checkpoint rows inside the same atomic transaction as the catalog update, then compacts substantial freed staging space after success. Version `0.145.17` also removes older completed-import staging leftovers at desktop startup, preserving incomplete/resumable sessions and import history. Startup cleanup and Aurora intake leave freed pages available for reuse rather than rewriting the entire database on every launch or small edit. A cancelled TSV preparation can resume only while the source path, size, and modification time still match the saved checkpoint; changed sources must be prepared again. The final Apply phase cannot be cancelled because it is one short SQLite transaction: success replaces the active snapshot, while any error rolls the whole transaction back. Generated folder snapshots, the legacy TSV, local `AlbumCovers/` archive, all eight local chart source folders, and local `MusicBrainz/` cache folder are intentionally ignored by git.
...
- Artist matches can be verified, ignored, unlinked, or corrected by pasting a MusicBrainz artist MBID. Unlink also works for unverified cache matches and suppresses the rejected automatic match until a replacement MBID is saved.
...
- The Tools workspace includes High-confidence missing MusicBrainz albums, Artists without MusicBrainz data, Missing Chart Albums, and Missing Chart Singles. Missing Chart Albums checks Billboard, Official UK, and VG Lista; Missing Chart Singles checks those sources plus Ti i Skuddet and Norsktoppen. Matching normalized artist/title appearances merge into one missing item while retaining a separate rank/year value for every source. Albums keep the earliest source year and singles keep the best source rank. CSV, TSV, XLSX, JSON, and text exports add explicit chart columns—Billboard, Official UK, VG Lista, Ti i Skuddet, and Norsktoppen as applicable—and an empty chart-entry table is prepared from its configured import folder when the tool is selected.
...
- **Your Daily Edition** turns Discovery into an editorial story front page sourced only from local evidence: 10-to-100-year owned-album anniversaries with up to five chart-ranked stories, synchronized ten-second cover rotation that restarts after era or thumbnail selection, exact MusicBrainz artist birthdays and memorials, an interactive owned-album chart shelf with Billboard year-end plus Official UK/VG-lista weekly source, year, week, and random snapshot controls, randomized Deep Cuts with one unrated, unloved, non-single track per highly rated album plus exact year, decade, genre, and refresh controls, and a randomized **Complete the Collection** shelf with Artists and Albums modes, exact year/decade/genre filters, MusicBrainz album gaps using the same cached per-release official-status verification as the main Artist discography, and owned albums that still contain unrated tracks. **Because You Played / Loved** mixes up to eight recent-rating or high-score/loved album anchors into six refreshable recommendations, prioritizes cached Last.fm related-album and similar-artist evidence, excludes every recent album and anchor artist, and only suggests albums with less than 50% of their tracks rated. Rating a track means it was played here, so conventional play counts are not required.
- The separate **Recommendation Mixer** combines two to eight user-selected local artists or albums without changing the single-page Similar Artists or Related Albums panels. Its Familiar-to-Explore control ranks locally owned albums using rating completeness, album score, and loved tracks; every result names the contributing seeds and relationship evidence. Results are deterministic, diversify seed coverage and artists, exclude every seed album and seed artist, collapse normalized duplicates, open directly in Albums, and read only the existing Last.fm caches plus canonical-genre fallback, so generating a mix is offline-safe and makes no provider request.
...
- **Source health** audits every Daily Edition input live: rating coverage and recent rating changes; Billboard, Official UK, and VG-lista album links; MusicBrainz artist identities and official release groups; Last.fm related-album and similar-artist caches; normalized genres; exact artist life dates; and cached cover art. Each source reports coverage, freshness, its exact last successful update, affected shelves, and concrete reasons a shelf may be sparse. Chart links can be safely rebuilt in place; other actions open the existing guarded Imports, MusicBrainz, Last.fm, or cover workflows. Archived editions stay unchanged and cannot be rebuilt from this view.
