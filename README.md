...
- The Tools workspace includes Missing Billboard Albums, which lists imported Billboard chart rows that are not linked to any library album. Overlapping chart-year entries for the same imported artist/title are collapsed to the earliest year. If the chart-entry table is empty after upgrading, selecting the tool prepares it from the default `CSV/` folder.
