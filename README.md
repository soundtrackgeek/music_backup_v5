...
The **Coverage Atlas** summarizes owned albums and open gaps by chart source and decade. Selecting a cell and choosing **Review candidates** fetches the complete source-and-decade cohort directly from SQLite, outside the global 5,000-row Workbench cap, and opens the exact **Open unverified** count as a focused campaign. MusicBrainz verification is deliberately manual and on demand: it searches with structured album-title and artist fields, excludes release groups carrying Live, Compilation, or another secondary type, and rechecks the selected pure Album for at least one Official release before it can be marked verified. Empty searches and provider failures remain visible with retry controls. Discogs is visible as the next provider phase but is not contacted in version 0.96.1, so Discogs credentials are not yet stored or required. SQLite schema version 40 stores completion decisions and their optional Wish List and MusicBrainz links.
