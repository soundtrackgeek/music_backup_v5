...
- An OpenAI API key is optional and only required for Ask Luna search, chart, current-view questions, Library analyst reports, and Playlist Builder recipes
...
## Luna Search, Charts, Current-view Questions, Library Analyst, and Playlists
...
- `src-tauri/src/ai.rs` owns OpenAI calls, strict query-plan/current-view/library-profile/playlist validation, typed Library analyst reports and playlist recipes, debug-only environment fallback, and Windows Credential Manager access; the frontend never receives the key.
