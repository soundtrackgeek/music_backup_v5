...
On **Imports**, choose one complete album folder after you have tagged, renamed, and placed it in its final library location. Music Library reads MP3 ID3 fields directly: title, artist, `DISPLAY ARTIST`, album artist, album, track/disc number, genre, publisher, year, native or legacy `TDRL` release year, `LOVE RATING`, MusicBee-owned POPM half-star rating, and duration. It does not search MusicBrainz or Discogs and it does not rename or move files.
...
Protocol version 1 uses file-based JSON operations: `capabilities`, `previewBatch`, `applyBatch`, and `syncExistingFolders`. A preview receives one already-tagged album folder or a parent whose immediate children are album folders, plus one category id. `general` resolves to `D:\MUSIC`, `scores` resolves to `G:\_BACKUP\SCORES` and is labeled **Movie / TV / game music**, and `synthwave` resolves to `H:\Synthwave`. The bridge never looks up metadata, changes genre or ratings, edits tags, or renames album folders. A single nested `Disc 1`/`CD1`-style child is treated as part of the selected album; a non-disc child beneath an inbox is treated as the album folder, so its actual album-folder name becomes the destination name.

Preview validates every album and produces one add-only whole-catalog delta. It rejects empty batches, more than 100 albums or 50,000 tracks, top-level MP3s mixed with child albums, multiple album identities in one folder, unsupported files, linked/reparse paths, case-insensitive destination collisions, occupied destinations, and source/destination overlap. It persists exact source and catalog fingerprints, SHA-256 hashes for every transferred file, final paths including disc subfolders, and the import session binding. Preparing a new bridge preview safely discards only an older incomplete bridge preview; completed receipts remain available for idempotent retry.

Apply revalidates the plan, catalog, source bytes, and destinations, then copies every album to an owned hidden staging folder on the destination volume. Every copied file is verified before the unchanged album folder is published. All sources remain in place until every destination is verified and the single existing staged import commits with its SQLite rollback backup. A pre-commit failure removes only exact, app-owned, unchanged copies; changed or unverifiable paths are retained for manual review. After commit, each unchanged source is atomically renamed to a sibling cleanup quarantine and removed. If a source changed during intake or cleanup cannot finish, the response reports `completedWithWarnings`, marks that source `retained`, and does not count it as moved.

`syncExistingFolders` accepts 1–32 canonical folders already present in the active catalog and synchronizes tag changes without moving files. It rejects paths outside configured library roots, linked/reparse paths, unsupported files, additions or removals, identity-set changes, and mutations outside the selected folders. The request has one protected pre-batch database baseline for complete rollback while each updated folder retains its normal import receipt. Exact Aurora-owned `.<MP3 filename>.aurora-<positive operation id>.original.backup` recovery files are ignored during scanning; other `.backup` files still fail closed.
